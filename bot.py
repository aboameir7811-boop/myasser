                ...
                for i, tf in enumerate(timeframes):
                    if results[i] and isinstance(results[i], list):
                        ...
                        if tf == '1h':
                            ...
                            # ==========================================
                            # 🔥 1. حساب بصمات الحيتان (Whale Net Flow & FVG)
                            # ==========================================
                            point_zero_idx = len(df_tf) - 1 # تحديد الفهرس الأخير للبيانات
                            
                            taker_buy_ratio = 1.0
                            whale_net_flow = 0.0
                            
                            # التأكد من وجود شموع كافية للحساب
                            if point_zero_idx >= 1:
                            ...
                            fvg_size_pct = 0.0
                            if point_zero_idx >= 2:
                                c1_high = float(df_tf.iloc[point_zero_idx - 2]['high'])
                                c1_low = float(df_tf.iloc[point_zero_idx - 2]['low'])
                                c3_high = float(df_tf.iloc[point_zero_idx]['high'])
                                c3_low = float(df_tf.iloc[point_zero_idx]['low'])
                            ...
                            # استخراج الدايفرجنس من دالة النماذج (إذا لم تكن موجودة سيضع NONE)
                            rsi_div = pattern_data.get("rsi_divergence_1h", "NONE")
                            obv_div = pattern_data.get("obv_divergence_1h", "NONE")
                        ...
                        # 🌟 [ استخراج النموذج الحالي فقط ] 🌟
                        pattern_name = detect_all_pdf_patterns(df_tf)
                        final_pattern = pattern_name if pattern_name else "لا يوجد نموذج"
                        ...
                        # 🗑️ تحديث بيانات الجدول الثاني (مختصر ونظيف تماماً)
                        record.update({
                            f"patterns_{tf}": final_pattern, 
...
async def forensic_investigation_cycle(active_investigations=None):
    """
    🕵️‍♂️ دورة المحقق الجنائي v11.3: تعمل بنظام العداد الزمني المستقل
    تحدث البيانات 👈 تفتش عن الانفجارات والانهيارات 👈 وتكرر العملية بانتظام.
    """
    if active_investigations is None:
        active_investigations = {}
        
    logging.info("✅ بدء تشغيل المحقق كونان بنظام العداد الزمني المستقل...")
    
    while True:
        try:
            print(f"\n🕵️‍♂️ [دورة جديدة للمحقق]: بدء التفتيش والتشريح... {datetime.now().strftime('%H:%M:%S')}")
            
            # 🔄 المرحلة الأولى: تحديث بيانات السوق لضمان دقة البيانات قبل التشريح
            print("📥 جاري تشغيل دالة تحديث بيانات السوق (update_crypto_market_data)...")
            await update_crypto_market_data()
            print("✅ اكتمل تحديث بيانات السوق بنجاح.")

            # 🔍 المرحلة الثانية: التفتيش الجنائي (الانفجارات والانهيارات)
            print("🚨 إطلاق حملة التفتيش الجنائي على منصة Binance...")
            current_time_ms = int(time.time() * 1000)
            
            # تنظيف الذاكرة: ننسى العملة بعد 24 ساعة لمنع التكرار المزعج
            keys_to_remove = [
                ...
                async with session.get("https://data-api.binance.vision/api/v3/ticker/24hr", timeout=10) as res:
                    if res.status == 200:
                        ...
                        for coin in tickers:
                            ...
                            # استبعاد العملات غير الصالحة والمستقرة
                            ...
                            # رصد الانفجارات (+30%) أو الانهيارات (-30%)
                            if vol > 50000 and (change >= 30 or change <= -30):
                                
                                # التحقق من الذاكرة لمنع تكرار التشريح لنفس العملة اليوم
                                if symbol not in active_investigations:
                                    active_investigations[symbol] = close_time_ms
                                    logging.info(f"🚨 [المحقق] رصد حدث جديد {symbol} بنسبة {change}%")
                                    
                                    # إرسالها للتشريح
                                    tasks.append(run_forensic_autopsy(symbol, change, close_time_ms))
                        
                        # التنفيذ المتوازي للتشريحات الجديدة فقط
                        if tasks:
                            await asyncio.gather(*tasks)
                            
            print(f"🏁 [المحقق] أنهى جولة التفتيش بنجاح. الذاكرة تحتوي على {len(active_investigations)} عملة.")
