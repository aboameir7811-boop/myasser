...
async def analyze_trend_structure_with_ai(report_id: str, symbol: str):
    ...
    try:
        ...
        update_payload = {
            "trend_structure_1h": analysis_data.get("trend_structure_1h", "لا توجد بيانات واضحة"),
            "trend_structure_2h": analysis_data.get("trend_structure_2h", "لا توجد بيانات واضحة"),
            "trend_structure_4h": analysis_data.get("trend_structure_4h", "لا توجد بيانات واضحة"),
            "trend_structure_1d": analysis_data.get("trend_structure_1d", "لا توجد بيانات واضحة")
        }
    ...
    try:
        ...
        if success_main:
            ...
            if all(results):
                ...
                # ==========================================
                # 👇👇 التعديل الآمن للخطوة الثانية 👇👇
                # ==========================================
                print(f"🧬 [تسليم الراية] جاري إرسال القضية ({report_id}) لغرفة الذكاء الاصطناعي...")
                asyncio.create_task(safe_ai_handover(report_id, symbol)) # 👈 استخدام غلاف الحماية
                # ==========================================
...
class TelegramLoggerHandler(logging.Handler):
    def __init__(self, bot, chat_id, loop):
        ...
        self.loop = loop # 👈 استقبال الـ Loop الرئيسي هنا

    def emit(self, record):
        log_entry = self.format(record)
        if record.levelno >= logging.ERROR:
            try:
                # 👈 إرسال المهمة للـ Loop بأمان تام من أي مكان في السكربت
                asyncio.run_coroutine_threadsafe(self.send_log(log_entry), self.loop)
            except Exception as e:
                print(f"⚠️ فشل النظام في توجيه اللوج للتليجرام: {e}")
...
async def main_startup():
    # 👈 التقاط الـ Loop الرئيسي للبرنامج
    main_loop = asyncio.get_running_loop()
    
    # 2. 🟢 إعداد اللوج مع تمرير الـ Loop
    logging.basicConfig(
        level=logging.INFO,
        format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
        handlers=[
            logging.StreamHandler(), # للطباعة في شاشة راندر كالعادة
            TelegramLoggerHandler(bot, GROUP_ID, main_loop) # 👈 تمرير الـ main_loop هنا
        ]
    )
